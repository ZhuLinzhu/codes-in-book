...
def OLS(in_X_train,in_y_train,in_X_oos, in_oos_data):
    ...
    datalr = inner_oos_data[['Dates','y','rethat']]
    datalr.columns = ['Dates','y','yhat']
    datalr['model'] = 'ols'
...
def EN(in_X_train,in_y_train,in_X_test,in_y_test,in_X_oos, in_test_data, in_oos_data):
    ...
    for a in alphas:
        ...
        Ytest_en = np.matrix(elasticnet.predict(in_X_test)).T
        ...
        Yoos_en = np.matrix(elasticnet.predict(in_X_oos)).T
        inner_oos_data = in_oos_data.copy()
        inner_oos_data['rethat'] = Yoos_en

        dataen = inner_oos_data[['Dates','y','rethat']]
        dataen.columns = ['Dates','y','yhat']
        dataen['model'] = 'EN' 
        dataen['AlphaValue'] = a 
...
def Ridge_method(in_X_train,in_y_train,in_X_test,in_y_test,in_X_oos, in_test_data, in_oos_data):
    ...
    for a in alphas:
        ...
        Ytest_ridge = np.matrix(ridge.predict(in_X_test)).T
        inner_test_data['rethat'] = Ytest_ridge
        res1 = 1-np.sum((inner_test_data['y']-inner_test_data['rethat'])**2)/np.sum(inner_test_data['y']**2) 
        ret_test.append(res1)

        
        Yoos_ridge = np.matrix(ridge.predict(in_X_oos)).T
        inner_oos_data = in_oos_data.copy()
        inner_oos_data['rethat'] = Yoos_ridge

        data_ridge = inner_oos_data[['Dates','y','rethat']]
        data_ridge.columns = ['Dates','y','yhat']
...
def Lasso_method(in_X_train,in_y_train,in_X_test,in_y_test,in_X_oos, in_test_data, in_oos_data):
    ...
    for a in alphas:
        ...
        Ytest_lasso = np.matrix(lasso.predict(in_X_test)).T
        inner_test_data['rethat'] = Ytest_lasso
        res1 = 1-np.sum((inner_test_data['y']-inner_test_data['rethat'])**2)/np.sum(inner_test_data['y']**2) 
        ret_test.append(res1)
        
        Yoos_lasso = np.matrix(lasso.predict(in_X_oos)).T
        inner_oos_data = in_oos_data.copy()
        inner_oos_data['rethat'] = Yoos_lasso

        data_lasso = inner_oos_data[['Dates','y','rethat']]
        data_lasso.columns = ['Dates','y','yhat']
...
def PLS_method(in_X_train,in_y_train,in_X_test,in_y_test,in_X_oos, in_test_data, in_oos_data):
    ...
    for k in range(1,9):  
        ...
        score = -1*cross_val_score(pls, in_X_train, in_y_train, cv=kf_10, scoring='neg_mean_squared_error', verbose=0).mean()
        mse.append(score)
...
def create_model(learn_rate=0.001, momentum=0.4,layer=3,dv = 0.2):###0.4relu
    ...
    if layer>=2:
        for i in range(layer-1):
            layer_i = 2**(i+1)
            model1.add(Dense(units = int(256/layer_i),kernel_initializer = 'random_normal' ,kernel_regularizer=regularizers.l2(1e-2)))
            model1.add(BatchNormalization())
            model1.add(Activation(gelu))
            model1.add(Dropout(dv))
        model1.add(Dense(1))
...
def NN(in_X_train,in_y_train,in_X_test,in_y_test,in_X_oos, in_test_data, in_oos_data):
    ...
    for l in range(2,6):
        ...
        data_nn = inner_oos_data[['Dates','y','rethat']]
        data_nn.columns = ['Dates','y','yhat']
        data_nn['model'] = 'NN' 
        data_nn['layers'] = l
...
def RandomForest_method(in_X_train,in_y_train,in_X_test,in_y_test,in_X_oos, in_test_data, in_oos_data):
    ...
    for n in n_es:
        ...
        Ytest_rf = np.matrix(rf.predict(in_X_test)).T
        inner_test_data['rethat'] = Ytest_rf
        res1 = 1-np.sum((inner_test_data['y']-inner_test_data['rethat'])**2)/np.sum(inner_test_data['y']**2) 
        ret_test.append(res1)

        
        Yoos_rf = np.matrix(rf.predict(in_X_oos)).T
        inner_oos_data = in_oos_data.copy()
        inner_oos_data['rethat'] = Yoos_rf

        data_rf = inner_oos_data[['Dates','y','rethat']]
        data_rf.columns = ['Dates','y','yhat']
        data_rf['model'] = 'rf' 
...
def GBDT_method(in_X_train,in_y_train,in_X_test,in_y_test,in_X_oos, in_test_data, in_oos_data):
    ...
    for n in n_es:
        ...
        Ytest_gbdt = np.matrix(gbdt.predict(in_X_test)).T
        inner_test_data['rethat'] = Ytest_gbdt
        res1 = 1-np.sum((inner_test_data['y']-inner_test_data['rethat'])**2)/np.sum(inner_test_data['y']**2) 
        ret_test.append(res1)

        
        Yoos_gbdt = np.matrix(gbdt.predict(in_X_oos)).T
        inner_oos_data = in_oos_data.copy()
        inner_oos_data['rethat'] = Yoos_gbdt

        data_gbdt = inner_oos_data[['Dates','y','rethat']]
        data_gbdt.columns = ['Dates','y','yhat']
